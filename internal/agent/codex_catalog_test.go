...
func TestCodexCatalogKeepsOwnEntries(t *testing.T) {
	...
	if err := json.Unmarshal(codexCatalog([]catalog.Model{
	...
	if third["slug"] != "deepseek/deepseek-chat" || third["base_instructions"] != codexPrompt || len(third["input_modalities"].([]any)) != 1 {
	...
	if own["base_instructions"] != codexPrompt {
